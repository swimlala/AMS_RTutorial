---
title: "Intro To R"
subtitle: "aka Why I Love R And You Should Too"
...
knitr::opts_chunk$set(echo = TRUE, fig.path = "figure/listings−",  cache =TRUE, options(width = 50), warning = FALSE, message = FALSE)
library(png)
library(tidyverse)
...
## Example Plot
```{r}
plot(starwars$height, starwars$mass, xlab = "Height", ylab = "Mass", main = "Star Wars Characters Mass vs Height", pch = 16, col = "blue")
```
...
colorado_messy <- colorado_covid %>% group_by(onset_dt , sex, age_group) %>% summarise(cases = n())



#colorado_clean <- separate(colorado_messy, `Race and ethnicity (combined)`, sep = ", ", into = c("race", "ethnicity"))

colorado_clean <- separate(colorado_messy, age_group, sep = " Years", into = c("age_group", NA))

colnames(colorado_clean)[1] <- "date"

colorado_clean <- colorado_clean %>% filter(age_group != "Unknown" & !is.na(age_group)) %>% drop_na()

colorado_clean  <- colorado_clean %>% filter(as.Date("2020-02-15") < date & date < as.Date("2020-07-01"))
...
include_graphics("./Images/welcome_to_rstats_twitter.png")
```
